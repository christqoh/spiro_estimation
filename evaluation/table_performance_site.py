...
def evaluate_performance_by_site(df: pd.DataFrame, path: str, run: str, n_iter: int):
    rows = ['Overall',
        ...
        for site in df.loc[:, 'site_id'].unique():
            for param in params:
                ...
                if r in ['Overall']:
                    ...
                    mask = (df.loc[:, 'cavity_present'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = - 2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, 'infiltrations'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = - 2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, 'lobar_volume_loss'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = - 2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, 'phenotype'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = -2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, '02_q_3a_dem_sex_at_birth'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = -2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, 'hiv_positive'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = -2.5
                    ch = None
                    tgt = df_s.loc[:, param + '_label'] > cut_low
                    ...
                    mask = (df.loc[:, 'severity'] == r) & (df.loc[:, 'site_id'] == site)
                    df_s = df.loc[mask, :]
                    cut_low = -1.64
                    cut_high = 10
    ...
    df_tgt.to_excel(path + '/model_performance_by_pathology_site_' + run + '.xlsx')
    df_tgt.to_csv(path + '/model_performance_by_pathology_site_' + run + '.csv')
...
def evaluate_performance_by_site_cutoff(df: pd.DataFrame, path: str, run: str, n_iter: int,
    ...
    rows = ['Overall',
            ...
            'Infiltration: True', 'Infiltration: False',
            'Loss of Lobar Volume: True', 'Loss of Lobar Volume: False',
            #'no_impairment', 'restriction', 'obstruction', 'mixed',
            'Female', 'Male', 'HIV Status: Positive', 'HIV Status: Negative']  #, 'none', 'mild', 'moderate', 'severe']
    params = ['fev1', 'fvc']
